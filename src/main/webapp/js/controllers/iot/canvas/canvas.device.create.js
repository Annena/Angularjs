app.controller('CavansDeviceCreate', ['$scope','$state','$http','point',function($scope,$state,$http,point) {
...
//    $scope.dev.monitorPointName = monitorpoint.name;
    ...
    $scope.selectType=function(){   
		...
		$scope.isTCP=false;
		$scope.isPPI=false;
    	if($scope.collectorType && $scope.collectorType != ''){
...
/* 根据名称获取己创建的网关信息  */
	$scope.getdeviceInfoByName = function(template){
		var methodurl = "/canvas/device/" +$scope.dev.monitorPointId +"/"+ $scope.dev.name;
		$.ajax({
        	method: "GET",
        	url:url+methodurl,
...
//    			alert('创建成功!!!!');
	...
	$scope.$on("CreateDeviceFromCanvas",function(event,msg){
		/* 记录当前正在创建的网关，如果创建成功，则返回canvas己经创建好的网关信息  */
		$scope.msg.node = msg;
		$scope.dev = {};
		$scope.dev.x = msg.x;
		$scope.dev.y = msg.y; 
		$scope.dev.name = msg.text;

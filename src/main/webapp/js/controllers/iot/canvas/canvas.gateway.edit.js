app.controller("CavansGatewayEdit",["$scope","$http","$state",function($scope,$http,$state){
...
//取网关的对应值	
	...
	$scope.getTheGateway = function(){
		...
		.success(function(data){
			...
			$scope.myAttrData = $scope.addList;
	...
	$scope.Add=function(){
	 ...
	 $scope.clickOK = function(row){
			...
			if(row.entity.name){
				$scope.isOK = true;
				var i = 0;
				angular.forEach($scope.myAttrData,function(item){
...
//确认修改按钮	
	$scope.Ok = function(){		
		...
		$scope.gateway.workgroupName = $scope.workgroup.name;
	...
	$scope.$on("EditGateWayFromCanvas",function(event,msg){
